...
nx::Class create PostController  -superclass Controller {
	...
	:public method actionCreate {} {
		...
		#	$model addError bla "expense [$model get expense]"
			set tags [$model get tags]
		#	puts "Tags for insert are $tags"
			$model unset tags
	...
	:public method actionUpdate {} {
		...
		if {[ns_conn method] == "POST"} {
			...
			set data [ns_striphtml [$model get post]]
			$model set reading_time [expr {round([string length $data]/5./200)}]
		#	puts "Reading time is [$model get reading_time] [llength $data]"
...
#return -level 100 or ns_adp_return which gives error..
	...
	:public method defaultNotFound {} {
		...
		#If nothing found by slug or id..
		#show first page of blog..
		set url [ns_conn url]
		set index 1

		if {${:urlLang} ne "na"} { set index 2 }
		set action [string tolower [lindex [join [split $url /]] $index]]
		set model [Post new]
		if {![string is integer $action]} {
			lappend conds [list slug $action]
		} else {
			lappend conds [list -cond AND id $action]
		}
	#	puts "Should find something with $conds"
			if {[$model findByCond -relations 1 $conds]} {
				if {[$model get cms]} {
				...
				my notFound	
				#correct redirection is done for /blog/ (index)
				#my redirect index
			}
	}
